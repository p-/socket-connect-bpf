...
func (t tableOutput) PrintHeader() {
	header := "%-12s %-14s %-6s %-32s %-16s %-16s %-20s %s\n"
	args := []interface{}{"TIME", "AF", "PID", "PROCESS", "USER", "PCOMM", "DESTINATION", "AS-INFO"}
...
func (t tableOutput) PrintLine(e eventPayload) {
	...
	header := "%-12s %-14s %-6d %-32s %-16s %-16s %-20s %s\n"
	args := []interface{}{e.Time, e.AddressFamily, e.Pid, e.ProcessPath, e.User, e.Comm, dest, asText}
	fmt.Printf(header, args...)
}
